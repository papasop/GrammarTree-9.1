# ============================================================
# GrammarForest + true L0 + PrimeGap v2 + AutoMetric (correlation only)
...
# -------------------------
# 4. L0：构建 (D^T W_E D - B) on scalar field |u|
# -------------------------
def build_L0_operator(Nx, Ny, Nz, device):
    print("[Build L0 graph operator] ...")
    V = Nx*Ny*Nz
    # 构造稀疏结构：6-neighbor periodic
    edges_i = []
    edges_j = []
    weights = []
    ...
    wE      = torch.tensor(weights, dtype=DTYPE, device=device)

    # incidence-like: 用拉普拉斯形式直接构造 L = D^T W_E D
    # 这里用 standard graph Laplacian：L = deg - A
    ...
    L0 = L - B
    print(f"[L0] Operator shape: {L0.shape}")
    return L0

L0_op = build_L0_operator(Nx,Ny,Nz,device)

def closure_residual_L0_scalar(field):
    """
    field: [B,Nx,Ny,Nz] (scalar, e.g. |u|)
    把每个 batch 拉成 [B,V]，应用 L0，返回 χ_rms
    """
    B = field.shape[0]
    V = Nx*Ny*Nz
    flat = field.view(B, V)
    ...
    # 这里等价于 χ = L0 u
    chi = Lu
    chi_norm = torch.sqrt((chi*chi).mean(dim=1) + 1e-20)
...
def rk2_step(v, rhs_fun, dt, proj=True, forcing=None):
    ...
    v_star = v + dt*k1
    if proj:
        vx_s,vy_s,vz_s,_ = project_incompressible(v_star[:,0],v_star[:,1],v_star[:,2],
                                                  dx,dy,dz,dt)
        v_star = torch.stack([vx_s,vy_s,vz_s], dim=1)
    k2 = rhs_fun(v_star, forcing=forcing)
    v_new = v + 0.5*dt*(k1 + k2)
    if proj:
        vx_n,vy_n,vz_n,_ = project_incompressible(v_new[:,0],v_new[:,1],v_new[:,2],
                                                  dx,dy,dz,dt)
...
class GrammarForestTau(nn.Module):
    """
    主体：a1*S + a2*I1S + 少量 forest 额外不变基（S2, R2, SR-RS, S3）
    """
    def __init__(self):
        super().__init__()
        # 主要两项 S, I1S
    ...
    def _tau_terms(self, v):
        ...
        # I1S
        I1S = torch.stack([I1*Sxx,I1*Syy,I1*Szz,I1*Sxy,I1*Sxz,I1*Syz], dim=1)
        # S2
        ...
        S3x = S2x*Sxx; S3y = S2y*Syy; S3z = S2z*Szz
        S3xy = S2xy*Sxy; S3xz = S2xz*Sxz; S3yz = S2yz*Syz
...
# -------------------------
# 10. 训练 GrammarForestTau (traj + struct + L0 + PrimeGap)
# -------------------------
def train_grammarforest_TGV():
    nu = 0.01
    teacher = TeacherTwoTermInvariant(a_tau=-4e-4, b_tau=-2e-4)
    v0_batch, F_batch, amps = build_dataset_TGV(n_train=4, amp_range=(0.45,0.60))
    ...
    chi_ref = closure_residual_L0_scalar(magT)
    ...
    # 训练阶段
    print("[Stage] Training GrammarForestTau (traj + struct + L0 + PrimeGap v2)...")
    ...
    for ep in range(1,max_ep+1):
        ...
        chi_M = closure_residual_L0_scalar(magM)
    ...
    with torch.no_grad():
        ...
        for n in range(Nt_eval):
            ...
            magM = torch.sqrt(vxM*vxM+vyM*vyM+vzM*vzM + 1e-16)
            chi = closure_residual_L0_scalar(magM)
    ...
    # AutoMetric: 一些预定义结构指标与 err(t) 相关性
    def corr(x,y):
        x = x - x.mean()
        y = y - y.mean()
        num = (x*y).sum()
...
# 11. 一键运行
# -------------------------
model, ts = train_grammarforest_TGV()
print("\n[Done] GrammarForestTau + true L0 + PrimeGap v2 + AutoMetric demo 完成。")
